...
notify() {
    ...
    _todo=$(cat ~/.local/share/plan/main.md | sed 's/\(- \[x\] \)\(.*\)/<span color="#ff79c6">\1<s>\2<\/s><\/span>/' | sed 's/- \[[ |x]\] //')
...
call_todo() {
    ...
    kill $pid1 && kill $pid2 || st -t statusutil_todo -g 50x15+$((mx - 200))+$((my + 20)) -c noborder -e nvim ~/.local/share/plan/main.md
}
